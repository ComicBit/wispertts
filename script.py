...
class Spinner:
    def __init__(self, desc): self.desc=desc; self.stop=threading.Event()
    def __enter__(self): log(self.desc); threading.Thread(target=self._spin,daemon=True).start(); return self
    def __exit__(self,*_): self.stop.set(); sys.stdout.write("\r"+" "*(len(self.desc)+4)+"\r"); sys.stdout.flush()
    def _spin(self):
...
def _dur(p):
    try: return float(subprocess.check_output(
        ["ffprobe","-v","error","-show_entries","format=duration","-of",
         "default=noprint_wrappers=1:nokey=1",p], text=True).strip())
    except Exception: return None

def _run(cmd: list[str], total: Optional[float], desc: str):
    if not total:
        with tqdm(total=1,desc=desc,bar_format='{l_bar}{bar}|{elapsed}',ncols=80) as pb:
            subprocess.run(cmd,stderr=subprocess.DEVNULL,check=True); pb.update(1); return
    with tqdm(total=int(total),desc=desc,unit="s",ncols=80) as pb:
        proc=subprocess.Popen(cmd,stderr=subprocess.PIPE,text=True)
        for line in proc.stderr:
            if (m:=T_RE.search(line)):
                h,mn,s=m.groups()
                pb.n=min(int(float(s)+int(mn)*60+int(h)*3600),pb.total); pb.refresh()
        proc.wait(); pb.n=pb.total; pb.refresh()
        if proc.returncode:
...
def remove_silence(p):
    ...
    _run(["ffmpeg","-y","-i",p,"-af",filt,dst],_dur(p),"[STEP] Removing silence")
    audio=AudioSegment.from_file(dst); os.unlink(dst); return audio

def fmt_ts(sec): h=int(sec//3600); m=int((sec%3600)//60); s=sec%60; return f"{h:02d}:{m:02d}:{s:05.2f}"

def chunk(audio):
    lim=SECONDS_LIMIT*1000; total=len(audio); start=0; chunks=[]
    with tqdm(total=(total//lim+1),desc="[STEP] Chunking",unit="chunk",ncols=80) as pb:
        while start<total:
            ...
            chunks.append((tmp,start/1000,end/1000)); start=end; pb.update(1)
    return chunks
...
def load_local(tag:str, dev:torch.device):
    if tag in _MODEL_CACHE: return _MODEL_CACHE[tag]
    if whisper is None: raise RuntimeError("Local mode requested but `openai-whisper` is not installed.")
    try:
    ...
    _MODEL_CACHE[tag]=mdl; return mdl
        ...
        try:
            with open(p,"rb") as f:
                return cli.audio.transcriptions.create(
                    ...
                    timeout=PER_REQ_TIMEOUT,language=lang).strip()
        except (openai.APITimeoutError,openai.RateLimitError) as e:
            back=2**i; log(f"[WARN] {e} – retry {i}/{tries} in {back}s"); time.sleep(back)
...
def transcribe_chunks(chs,*,mode,cli=None,mdl=None,lang):
    ...
    with tqdm(total=len(chs),desc="[STEP] Transcribing",unit="chunk",ncols=80) as pb, \
         ThreadPoolExecutor(MAX_CONC_REQUESTS if mode=="api" else 1) as pool:
        for fut in as_completed(pool.submit(work,x) for x in enumerate(chs)):
            i,*row=fut.result(); res[i]=tuple(row); pb.update(1)
    return res
...
def diarize_tx(audio,*,mode,cli=None,mdl=None,lang):
    ...
    try:
        ...
        raise RuntimeError(f"[FATAL] Could not load diarization pipeline:\n{e}") from None
    
    if pipe is None:
        raise RuntimeError("[FATAL] Diarization pipeline failed to load")

    # progress hook optional
    try: from pyannote.audio.pipelines.utils.hook import ProgressHook
    except ImportError: ProgressHook=None

    wav=tempfile.NamedTemporaryFile(delete=False,suffix=".wav").name
    audio.export(wav,format="wav")
    device=prefer_device(); pipe.to(device); log(f"[STEP] Diarization on {device}")

    try:
        if ProgressHook:
            with ProgressHook() as hook: diar=pipe(wav,hook=hook)
        else:
            with Spinner("[STEP] Diarization in progress"): diar=pipe(wav)
    finally:
        os.unlink(wav)
    ...
    for turn,_,sp in diar.itertracks(yield_label=True):
        ...
        spk_map.setdefault(sp,f"Speaker {spk_idx}"); spk_idx=len(spk_map)+1
        segment=full[int(turn.start*1000):int(turn.end*1000)]
        for off in range(0,len(segment),limit):
            part=segment[off:off+limit]; st=turn.start+off/1000; ed=min(turn.end,st+len(part)/1000)
            jobs.append((order,spk_map[sp],part,st,ed)); order+=1
    ...
    def work(j):
        ...
        try: txt=w_tx_api(cli,tmp,lang) if mode=="api" else w_tx_local(mdl,tmp,lang)
        finally: os.unlink(tmp)
        return i,sp,txt,st,ed

    with tqdm(total=len(jobs),desc="[STEP] Transcribing segments",unit="seg",ncols=80) as pb, \
         ThreadPoolExecutor(MAX_CONC_REQUESTS if mode=="api" else 1) as pool:
...
def merge(rows):
    ...
    for sp,tx,st,ed in rows[1:]:
        sp0,tx0,_,ed0=m[-1]
        if sp==sp0: m[-1][1]=f"{tx0} {tx}"; m[-1][3]=ed
        else: m.append([sp,tx,st,ed])
    return [tuple(r) for r in m]

def to_lines(rows,show): return [f"{f'[{fmt_ts(st)}-{fmt_ts(ed)}] ' if show else ''}{f'[{sp}] ' if sp else ''}{tx}" for sp,tx,st,ed in rows]

# ------------------------------ ORCHESTRATE ---------------------------------
def run(infile,outfile,*,mode,local_tag,diarize,lang,show_ts,agg):
    cli=mdl=None
    if mode=="api":
        if openai is None: raise RuntimeError("openai package missing.")
        if OPENAI_API_KEY=="your-api-key-here": raise RuntimeError("Set OPENAI_API_KEY")
        cli=openai.OpenAI(api_key=OPENAI_API_KEY,
                          timeout=httpx.Timeout(120,read=PER_REQ_TIMEOUT),max_retries=3)
    else:
        mdl=load_local(local_tag,prefer_device())

    mp3=convert(infile); cleaned=remove_silence(mp3)
    rows=(diarize_tx(cleaned,mode=mode,cli=cli,mdl=mdl,lang=lang) if diarize else
          transcribe_chunks(chunk(cleaned),mode=mode,cli=cli,mdl=mdl,lang=lang))
    if agg: rows=merge(rows)
    with open(outfile,"w",encoding="utf-8") as fh: fh.write("\n".join(to_lines(rows,show_ts)))
    log(f"[SUCCESS] Transcript → {os.path.abspath(outfile)}")

# ------------------------------ CLI -----------------------------------------
def main():
    ap=argparse.ArgumentParser(description="Transcribe audio via Whisper (API/local).")
    ap.add_argument("input_file"); ap.add_argument("output_file")
    ...
    run(args.input_file,args.output_file,
        mode=args.mode,local_tag=args.local_model,diarize=args.diarize,
        lang=args.language,show_ts=args.timestamps,agg=args.aggregate)

if __name__=="__main__":
    log(f"[BOOT] {sys.argv}"); main()
